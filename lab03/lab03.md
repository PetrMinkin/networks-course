...
2. Первый пакет придет в буфер через $\dfrac{L}{R_S} + \dfrac{L}{d}$ секунд и покинет буфер через $\dfrac{L}{R_C}$ секунд. Второй пакет отправляется сразу после первого, то есть через $\dfrac{L}{R_S}$ секунд от начала передачи. Ему придется ждать в буфере, если $\dfrac{L}{R_S} + \dfrac{L}{d}+\dfrac{L}{R_C} > \dfrac{L}{R_S}+\dfrac{L}{R_S} + \dfrac{L}{d}$, то есть если $R_S > R_C$. Задержка $T$ будет увеличивать время с момента начала отправки пакетов и до прибытия второго пакета в буфер, значит, чтобы очередь была нулевая, она должна быть равна $T = \dfrac{L}{R_S} + \dfrac{L}{d}+\dfrac{L}{R_C} - \dfrac{L}{R_S} - \dfrac{L}{d} = \dfrac{L}{R_C}$.
