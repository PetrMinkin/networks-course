...
#### Решение
Пакет в $56$ байт весит $56 \cdot 8 = 448$ бит. Каждый бит должен дождаться создания пакета с собой, это происходит от $0$ милисекунд до $\dfrac{448}{128\cdot 1000} = 0.0035 = 3.5 $ милисекунд (сборка пакета с нуля). Затем пакет передается за $\dfrac{448}{10^6} = 0.448$ мс. Вместе с задержкой распространения итоговая задержка составляет от $5.448$ до $8.948$ милисекунд.
...
#### Решение
$N=10+1=11$. Если скорость передачи составляет $100$ пакетов в секунду, то задержка передачи составляет $\frac{1}{100} = 10$ мс. Тогда $d = 20$ мс. Значит по формуле Литтла $a = 11/20 = 0.55 $ пакетов в милисекунду.
...
#### Решение
1. Поскольку узким местом является первая линия связи, то нигде не будет возникать очереди пакетов. Тогда второй пакет будет отправлен сразу после первого, то есть через $\dfrac{L}{R_S}$ секунд, и пройдет между сервером и клиентом за то же время чо и первый пакет.

2. Первый пакет придет в буфер через $\dfrac{L}{R_S} + \dfrac{L}{d}$ секунд и покинет буфер через $\dfrac{L}{R_C}$ секунд. Второй пакет отправляется сразу после первого, то есть через $\dfrac{L}{R_S}$ секунд от начала передачи. Ему придется ждать в буфере, если $\dfrac{L}{R_S} + \dfrac{L}{d}+\dfrac{L}{R_C} > \dfrac{L}{R_S}+\dfrac{L}{R_S} + \dfrac{L}{d}$, то есть если $R_S > R_C$. Задержка $T$ будет увеличивать время с момента начала отправки пакетов и до прибытия второго пакета в буфер, значит, чтобы очередь была нулевая, она должна быть равна $T = \dfrac{L}{R_S} + \dfrac{L}{d}+\dfrac{L}{R_C} - \dfrac{L}{R_S} - \dfrac{L}{d} - \dfrac{L}{R_S} = \dfrac{L}{R_C}- \dfrac{L}{R_S}$.
...
#### Решение
1. $\Delta = \dfrac{0.85}{15} = 0.0567$ секунд, так как скорость отправки ЛВС огромная, там задержек не будет.
2. Среднее время ответа $\dfrac{  0.0567}{1- 16\cdot 0.0567} + 3= 3.601$ секунд.
3. Если коэффициент непопадания в кэш равен $0.4$, то $B = 16\cdot 0.4 = 6.4$ запр/с. Тогда среднее время ответа равно $3+\dfrac{  0.0567}{1- 6.4 \cdot 0.0567} = 3.089$ секунд.
