...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - 0
   - Там стоит флаг 0x002 что это SYN сегмент.
   - ![](images/2.png)
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - Сервер взял порядковый номер у пришедшего пакета и увеличил на 1
   - Стоит флаг 0x012, показывающий SYNASK сегмент
   - ![](images/3.png)
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - Номера: 1, 1335, 2669, 4003, 5337, 6671
   - Времена отправки у всех: 2.843399
   - ![](images/5.png)
   - Времена получения ответов на все получаются: 3.053108
   - ![](images/6.png)
   - RTT = 0.209718 секунд для всех сегментов
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - ![](images/7.png)
   - Посчитаем сколько всего мы байт отправили мы отправили порядка 150000 байт за $3.372990 - 2.840386 = 0.532604$ секунд. То есть пропускная способность канала равна $150000/0.532604=281 816$ байт/с. 
